...
const FACE_MATCH_THRESHOLD = 0.6; // Adjust this threshold as needed
...
const TakeAttendance = () => {
  ...
  const captureNow = async () => {
    ...
    // Save last captured image for UI (only while camera is active)
  ...
  const stopCamera = () => {
    if (videoRef.current && videoRef.current.srcObject) {
      ...
      setLastCapturedImage(null); // Remove last captured image when camera stops
    }
  };
  ...
  const saveAttendance = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success(`Attendance saved for ${recognizedStudents.length} students`);
      setRecognizedStudents([]);
      setCameraActive(false);
      navigate('/');
    } catch (error) {
                ...
                <div className="relative">
                  ...
                  {cameraActive && lastCapturedImage && (
                    <img
                      ...
                      alt="Last captured"
                      className="absolute top-0 left-0 w-full h-96 object-cover rounded-lg border-2 border-blue-400 z-10 pointer-events-none"
                      style={{ opacity: 0.5 }}
                    />
                  )}
